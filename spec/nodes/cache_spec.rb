describe Travis::Yaml::Nodes::Cache do
  context 'from Ruby' do
    specify 'with true' do
      ...
      expect(config.cache).to be == { 'apt' => true, 'bundler' => true, 'cocoapods' => true, 'pip' => true,
                                      'ccache' => true }
    end

    specify 'with false' do
      config = Travis::Yaml.parse(cache: false)
      expect(config.cache).to be == { 'apt' => false, 'bundler' => false, 'cocoapods' => false, 'pip' => false,
                                      'ccache' => false }
    end
    ...
    specify 'with array' do
      config = Travis::Yaml.parse(cache: %w[apt bundler])
      expect(config.cache).to be == { 'apt' => true, 'bundler' => true }
    end

    specify 'with hash' do
      config = Travis::Yaml.parse(cache: { bundler: true, directories: %w[a b] })
      expect(config.cache).to be == { 'bundler' => true, 'directories' => %w[a b] }
    end
  end
