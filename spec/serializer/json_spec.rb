describe Travis::Yaml::Serializer::Json do
  ...
  example 'does work properly with symbol keys option' do
    expect(config.serialize(:json,
                            symbol_keys: true)).to be == '{"env":{"matrix":[{"secure":"foo"},"bar"]},"language":"ruby","os":["linux"]}'
  end
  ...
  example 'complains about decrypted values missing' do
    expect do
      config.serialize(:json,
                       secure: :decrypted)
    end.to raise_error(ArgumentError, 'secure option is set decrypted, but a secure value is not decrypted')
  end

  example 'serializes decrypted values' do
    config.decrypt { |*| 'x' }
    expect(config.serialize(:json,
                            secure: :encrypted)).to be == '{"env":{"matrix":[{"secure":"foo"},"bar"]},"language":"ruby","os":["linux"]}'
    expect(config.serialize(:json,
                            secure: :decrypted)).to be == '{"env":{"matrix":["x","bar"]},"language":"ruby","os":["linux"]}'
  end
