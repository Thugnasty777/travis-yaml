...
module Travis::Yaml
  ...
  module Nodes
    ...
    class FixedValue
      def self.spec_description(*prefix)
        super || begin
          list = valid_values.map do |v|
            "`#{v}`#{' (default)' if default == v.to_s}"
          end.join(', ').gsub(/,([^,]+)$/, ' or\1')
          if aliases.any?
    ...
    class Sequence
      def self.spec_format(_append = '', children = true, *)
      ...
      def self.spec(*prefix, **options)
        specs = super
        specs += type.spec(*prefix, '[]') unless (type <= Scalar) && (type >= FixedValue)
        specs
      end
    end
    ...
    class Mapping
      ...
      def self.spec_format_prefix(input, append = '')
        if prefix_sequence
          input << ", or #{subnode_for(prefix_sequence).spec_format('s',
                                                                    prefix_sequence != prefix_scalar)}"
        end
        if prefix_scalar && (prefix_scalar != prefix_sequence)
          input << ", or #{subnode_for(prefix_scalar).spec_format(append)}"
        end
      end
      ...
      def self.spec_options(key, **_options)
        if (self < LanguageSpecific) && (languages = LanguageSpecific::LANGUAGE_SPECIFIC[key.to_sym])
          languages = languages.map do |v|
            "`#{v}`#{' (default)' if v.to_s == 'ruby'}"
          end.join(', ').gsub(/,([^,]+)$/, ' or\1')
        end
      ...
      def self.spec(*prefix, **options)
        specs = mapping.sort_by(&:first).inject(super) do |l, (k, v)|
          l + v.spec(*prefix, k, **spec_options(k, **options))
        end
